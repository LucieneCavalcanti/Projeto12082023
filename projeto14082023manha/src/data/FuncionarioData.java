...
public class FuncionarioData extends Conexao implements CRUD {
    ...
    public boolean incluir(Funcionario obj) throws Exception {
        CallableStatement callable = getConexao().prepareCall("{call cadastrarFuncionario (?,?,?, ?)}");
        ...
        int registros = callable.executeUpdate(); // executa o comando no BD
        if (registros == 1)
            return true;
        else
            return false;
    ...
    public Funcionario obter(int id) throws Exception {
        ...
        if (dados.next()) {
            obj = new Funcionario(dados.getInt("id"), dados.getString("nome"), dados.getString("email"),
                    dados.getString("senha"), dados.getString("cargo"));
        }
        return obj;
    }
